...
*** Test Cases ***
Create Merchant Successfully
    [Documentation]    Create a merchant with valid data and a valid access token.
    Get Token
    ${resp}=    Create Merchant And Return Response
...
Create Merchant With Invalid Token
    [Documentation]    Create a merchant with valid data and an invalid access token.
...
Create Merchant With Empty BaseCurrency
    [Documentation]    Create new Merchant using an empty baseCurrency value.
    Get Token
    ${resp}=    Create Merchant And Return Response    ${EMPTY}    INVxxxxxx    30    1    80-01-01    SHOPPING & RETAIL    true
    ${resContent}=    Set Variable    ${resp.json()}
